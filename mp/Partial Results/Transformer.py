...
class Transformer:
    ...
    def __call__(self, source_seq, target_seq):
        ...
        print(f"source vocab embedding = {self.source_embedding.vocab_embedding.shape}")
        print(f"target vocab embedding = {self.target_embedding.vocab_embedding.shape}")
    ...
    def backward(self, source_seq, target_seq, output_probs, target_labels):
        ...
        # FOR DEBUGGING
        print("SHAPES HERE SHOULD MATCH WITH TRANSFORMER OUTPUT SHAPES:")
        print(f'grad output logits = {grad_output_logits.shape}')
        print(f'grad softmax = {grad_softmax.shape}')
        print(f'grad final linear layer = {grad_final_linear_layer.shape}')
        print(f'grad decoder query block = {grad_decoder_block[1].shape}')
        print(f'grad decoder key block = {grad_decoder_block[0].shape}')
        print(f'grad encoder block = {grad_encoder_block.shape}')
        print(f'grad source vocab embedding = {grad_source_embedded.shape}')
        print(f'grad target vocab embedding = {grad_target_embedded.shape}\n')
